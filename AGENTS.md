...
- **Attendance:** `PRESENT ≤08:00`, `LATE` otherwise, `DUPLICATE` on same-day re-scan, `NOT_SCHEDULED` muted never absent, `ABSENT` only via `POST /api/reconcile` after `lateCutoff` with today guard `BEFORE_CUTOFF`.

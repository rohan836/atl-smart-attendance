...
- **Current behavior (2026-08-31):** terminal empty `PLACE YOUR FINGER TO SCAN` + Admin; served page uses `backend/ui_app.js` and actively `POST /api/scan {waitSec:2}` when Admin closed. Enrollment from Students → New Enrollment, one Start + 3 lifts. LocalStorage cache omits photos.
- **Admin UX (2026-08-31):** Calendar schedule-only (global weekly + **backend-persisted** per-class/batch overrides via `classSchedules/batchSchedules` + holidays/overrides global, month view; exam days count as working); Settings single source school info + classes; Students toolbar **New Enrollment** + Batch/Status filters + re-activate (restores roll) + clear photo + full CSV; Today `Not Scheduled` muted never Absent; Reports custom validated; Backup DB + audit CSV; Esc + backdrop close (aborts enroll poll).

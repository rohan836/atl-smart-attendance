...
On page load `backend/ui_app.js` calls `cacheLoad()` from LocalStorage, then `loadAll()` fetches SQLite truth: `GET /api/settings`, `GET /api/students?active=all`, `GET /api/attendance`, `GET /api/attendance?date=today`, `GET /api/daily?date=today`, `GET /api/kpis?date=today`, and `GET /api/audit`. Results map through `mapStudent()` and `mapEvent()` and are cached via `cacheSave()` with photos stripped. The same refresh runs every 15 seconds while the tab is visible. Dates are `YYYY-MM-DD` local in both layers (`today_ist()` IST on backend, `todayISO()`/`toLocalISO()` `getFullYear/Month/Date` on frontend; `renderCalendarMonth` and report `week`/`month` use `toLocalISO` not `toISOString()`). Opening the app as `file://` is unsupported; it must be served from Flask at `/`.

## Scan cycle — terminal idle and background Admin scanning

The UI is DB-driven. When the enroll modal and exclusive operations are closed and no result is held, `sensorScanLoop()` in `ui_app.js` posts `POST /api/scan {waitSec:2}`. The backend in `app.py` acquires `SENSOR_LOCK`, calls `GT511C3.identify(timeout=waitSec)`, and waits for a finger via `wait_finger()` then `Capture` then `Identify`. Three outcomes create no persistence: `NO_FINGER` (400), `SENSOR_BUSY` (503), and UART errors return without writing `events` or `daily`. A parallel bridge poller `GET /api/scan/last` every 2 seconds (injected `SCAN_BRIDGE_SCRIPT`) provides a fallback; both paths call `window.handleRealScan(fid, {status,time,date,seq,student})`. When the Admin panel is open (especially on Attendance), scans are processed into SQLite and update the UI table live while the full-screen identity popup is suppressed. Scanning pauses exclusively during enrollment (`enrollModal`/`scanModal`), re-enrollment, student deletion, and database restore.

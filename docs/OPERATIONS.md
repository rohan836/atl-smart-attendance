...
- **Architecture:** Personal Google Drive using OAuth 2.0 with the least-privilege `https://www.googleapis.com/auth/drive.file` scope (only accesses files/folders created by this application; cannot read the user's other Drive contents). Uses standard chunked Resumable Upload protocol (1MB chunks) with exponential backoff for network resilience. Pure Python standard library implementation (`urllib.request` + `sqlite3.backup()` + `hashlib`).
...
- **Automated daily schedule:** Background daemon (`_gdrive_backup_daemon`) runs every minute. After attendance reconciliation and daily cutoff, once local time reaches `scheduleTime` (default `18:30` IST), takes an online snapshot and uploads to Google Drive.

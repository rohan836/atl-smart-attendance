...
- **main (Current baseline):** Unified Attendance workspace (live Today default, single-student reporting & authoritative `/api/reports?studentId` metrics, custom range controls with Apply action, streaming multi-day CSV export, editorial single & multi-day Print) + selective background attendance scanning during Admin (live table & KPI updates with fullscreen popup suppression, exclusive pause during enrollment and maintenance) + Unified Setup UI (school rules, classes/batches, Global/Class/Batch schedule contexts, Month View) + Unified Backup Manager (Google Drive, Telegram, USB). Tests: `116/116` backend tests + `14/14` Playwright browser E2E tests passing (`130/130` total).

...
Behavior lives in `backend/app.py` (Flask + scheduling + validation), `backend/gt511c3.py` (UART protocol), `backend/schema.sql` (tables), and `backend/ui_app.js` (UI logic). `ATL-Smart-Attendance-Production.html` is a CSS and markup shell; its `<script>` block is replaced at serve time by `_serve_production()` in `app.py:477`. Edit `ui_app.js` for any behavior or Admin change; do not edit the HTML splice stub. The GT-511C3 template store plus SQLite are truth; LocalStorage `atl_*` is a photos-omitted cache.

## What not to touch

Never commit or deploy `backend/config.json`, any `*.db`, `*.pre_restore.bak`, `backend/uploads/`, `__pycache__/`, `*.log`, `.venv/` or `.env` — they are gitignored and contain machine-specific or derived data. Never deploy the development database to the Pi. Keep dead routes `/legacy|/terminal|/perfect|/css|/js` removed. Never store `sensor/uart/baud/db/host/port/imagesDir` through `POST /api/settings` — they are file-only and excluded by the whitelist in `app.py:580`. Never power the sensor from 5V; VCC is 3.3V pin 1 only. Keep `keep_led_on=True` in `app.py:62` — the LED stays on while the service runs.

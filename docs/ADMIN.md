...
Backup supports four recovery and offsite layers:
...
2. **Google Drive Cloud Backup:** Automated offsite backups to personal Google Drive using OAuth 2.0 Device Authorization Grant (RFC 8628) with least-privilege `drive.file` scope. The administrator connects directly via Google Device Flow (navigating to `google.com/device` and entering the code displayed in the UI), configures versatile automatic backup schedules directly from the UI (backup time, daily/every N days/specific weekdays, or enable/disable), triggers immediate cloud backups ("Backup to Drive now"), views historical snapshots in `ATL-Attendance-Backups`, and restores from cloud backups (never automatic, operator-initiated only). Backups run automatically via background worker with Grandfather-Father-Son retention (7 daily, 4 weekly, 12 monthly).
3. **Telegram Secondary Cloud Backup:** Secondary offsite destination that uploads verified SQLite backup snapshots directly to a designated Telegram chat or channel as a document via official Telegram Bot API `sendDocument`. Operates asynchronously and non-blocking — Telegram failures never disrupt local or Google Drive backup routines. The Admin Backup tab provides an enable toggle, configuration and connectivity status, "Send backup now" trigger, status clear action, and an independent Automatic Backup Schedule (backup time, daily / every N days / specific weekdays, enable/disable). Bot tokens remain protected in server configuration and are never exposed via UI, logs, or API responses.
4. **USB Local Storage Backup:** Dedicated offline physical backup layer that detects attached USB storage drives and writes verified SQLite snapshots directly into `ATL-Attendance-Backups/atl_backup_*.db`. Automatically creates scheduled backups when the configured schedule arrives and a USB drive is attached, with same-day duplicate prevention. If no USB device is connected, the background worker exits cleanly and never impedes attendance or cloud operations. The Admin Backup tab provides an interactive enable toggle, mount point detection, free disk space reporting, "Backup to USB now" manual trigger, "Check USB", and a full schedule configuration suite (time, frequency, interval, weekdays).
